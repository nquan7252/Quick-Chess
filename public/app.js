...
socket.emit("move",chosenPieceName,chosenPieceColor,chosenPiece,button[i].id,chosenPieceSrc)
    clicked=!clicked;
});
}
...
function checkValidMoveKing(oldposition,newposition){
    ...
    if ((newposition.includes(point1)||newposition.includes(point2)||newposition.includes(point3)||newposition.includes(point4)||newposition.includes(point5)||newposition.includes(point6)||newposition.includes(point7)||newposition.includes(point8))&&isEmpty(newposition)){
        return true;
    }
    return false;
}
function checkValidMovePawnBlack(position1,position2,isFirstTime){
   ...
   function checkGameOver(){
    ...
    function stopGame(){
        ...
        var messageReset=confirm("Play again");
        if (messageReset){
            console.log("Stop and initialize");
            initialize();
        }
    }
function updateTurn(){
